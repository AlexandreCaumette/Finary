...
class Cube:
    ...
    def on_select_estate_source(self):
        selected_rows_index = state.read_state('estate_sources_dataframe')['selection']['rows']
        
        if len(selected_rows_index) == 0:
            state.write_state(key='index_estate_source_selected', value=None)
            
            state.write_state(key='input_estate_source_type',
                         value=None)
        
            state.write_state(key='input_estate_source_label',
                            value=None)
            
            state.write_state(key='input_estate_source_amount',
                            value=0.)
            
            state.write_state(key='input_estate_source_deposit',
                            value=0.)
            
            state.write_state(key='input_estate_source_limit',
                            value=None)
            
            state.write_state(key='input_estate_source_return',
                            value=0.)
        else:
    ...
    def is_income_source_selected(self) -> bool:
        selected_rows_index = state.read_state('income_sources_dataframe')['selection']['rows']
    ...
    def on_select_income_source(self):
        ...
        if len(selected_rows_index) == 0:
            ...
            state.write_state(key='input_income_source_label',
                         value=None)
        
            state.write_state(key='input_annual_gross_salary',
                            value=None)
            
            state.write_state(key='input_annual_net_salary',
                            value=0.)
            
            state.write_state(key='input_annual_net_salary_after_tax',
                            value=0.)
            
            state.write_state(key='input_average_annual_increase',
                            value=None)
        else:
            index_source = selected_rows_index[0]
            ...
            state.write_state(key='income_source_type_selection',
    ...
    def get_estate_projection(self, years: int = 10) -> pl.DataFrame:
        ...
        def compute_future_amounts(montant_initial, estate_source_part, rendement, years, limit: float = None): 
