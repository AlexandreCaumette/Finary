...
def money_metric(parent: DeltaGenerator, label: str = '', amount: float = 0, help: str = None):
    parent.metric(label=label,
                  value=f"{amount:,.2f} €",
                  help=help)
...
def render_metric_estate(parent: DeltaGenerator, years: int = 0):
    money_metric(parent=parent,
                 ...
                 help="Cette indicateur est calculé en additionnant le montant de toutes les sources de patrimoine.")
